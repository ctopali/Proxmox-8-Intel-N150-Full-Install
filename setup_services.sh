...
create_lxc_from_vars() {
    source /usr/local/community-scripts/defaults/cloudflared.vars

    TEMPLATE=$(find /var/lib/vz/template/cache -maxdepth 1 -type f \
        -name 'debian-13-standard*.tar.zst' | sort -V | tail -n1)
    
    if [[ -z "$TEMPLATE" ]]; then
        echo "No se encontró ningún template Debian 13."
        exit 1
    fi
    
    echo "Usando template: $(basename "$TEMPLATE")"
